    ...
    <b-collapse v-for="(rating_attrs, rating) in ratings" class="card" :open.sync="rating_attrs.isOpen">
      <div slot="trigger" slot-scope="props" class="card-header" role="button">
        ...
        <div class="card-header-title" v-for="move in rating_attrs.moves.slice(0, 3)">
          <b-button v-show="!rating_attrs.isOpen" @click.stop="roll(move)" size="is-small" type="is-primary">{{move.name}}</b-button>
        </div>
      ...
      <div class="card-content">
        <div class="content">
          <div v-for="move in rating_attrs.moves">
            <div class="level">
              {{move.name}} <b-button @click="roll(move)" type="is-primary">Roll</b-button>
            </div>
