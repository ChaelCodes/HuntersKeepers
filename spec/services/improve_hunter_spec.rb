...
RSpec.describe ImproveHunter do
  ...
  let(:hunters_improvement) do
    build(:hunters_improvement, improvement: improvement, hunter: hunter)
  end
  ...
  describe '.for' do
    subject { described_class.for(improvement) }

    let(:improvement) { build :another_move }

    it 'return the Improves Service for the improvement' do
      is_expected.to eq(Improves::AnotherMove)
    end
  end
end
